...
{
    public class XrLepestok : XRBaseInteractable
    {
       ...
       public void Povorot()
        ...
        public void EndGrab(SelectExitEventArgs arts)
        {
            UpdateValue();
            if(end == true)
            {
        ...
        void UpdateValue()
        {
            ...
            if (Mathf.Abs(leftRightAngle) > Mathf.Abs(upDownAngle))
            {

                if (Mathf.Abs(leftRightAngle) >= m_DeadZoneAngle)
                {
                    if (leftRightAngle > 0)
                        leftRightAngle = m_MaxAngle;
                    else
                        leftRightAngle = -m_MaxAngle;
            ...
            else
            {
                ...
                if (Mathf.Abs(upDownAngle) >= m_DeadZoneAngle)
                {
                    if (upDownAngle > 0)
                        upDownAngle = m_MaxAngle;
                    else
                        upDownAngle = -m_MaxAngle;
